...
client.on("message", message => {
  ...
  const wordChannels = ["766737470842208287"];

  //characters toni wants in his words

  const leagalCharacters = [
    "a",
    "b",
    "c",
    "d",
  ...
  if (Date.now() - userStats.last_message >= 60000) {
    ...
    message
      ...
      .then(collected => {
        ...
        if (reaction.emoji.name === "⏺️") {
          ...
          const amount =
            100 - Math.ceil((Date.now() - userStats.last_message) / 100);
          ...
          message
            .react(numbers[tens])
            .then(() => message.react(numbers[ones + 10]))
    ...
    message
      .awaitReactions(filter2, { max: 1, time: 10000, errors: ["time"] })
    ...
    if (userStats.xp >= xpToNextLevel) {
      ...
      message.channel.send(
        message.author.username + " has reached level " + userStats.level
      );
    }
    ...
    console.log(
      "It's been " +
        (Date.now() - userStats.last_message) / 1000 +
        " Seconds since last message"
    );
    console.log("");

    const toni = speedStats["763245951573688350"];
    userSpeedStats.average =
      toni.total_score / toni.total_times -
      (100 - userSpeedStats.total_score / userSpeedStats.total_times);
